...
export default class goodsBindSupplier extends Component {
    ...
    _clickRowSelection = (text) => {
        ...
        text.product_num = this.state.productDetail.product_num
        if (check !== -1) return;
    ...
    _clickRowSelectionAll = () => {
        ...
        this.state.selectedRowKeys.forEach(e => {
            ...
            e.product_num = this.state.productDetail.product_num
    ...
    //-- 删除已选择的供应商
    _delVendorList = (index) => {
        const { selectedVendorList } = this.state
        selectedVendorList.splice(index, 1);
        this.setState({ selectedVendorList: selectedVendorList })
    }
                            ...
                            </div>
                            {
                                this.state.selectedVendorList.map((e, i) => (
                                    <div style={{ width: '100%', height: 40, display: 'flex', justifyContent: 'space-around', alignItems: 'center', border: '1px solid #efefef', borderTop: '0px' }} key={i}>
                                        ...
                                        <span onClick={() => this._delVendorList(i)} style={{ flex: 1, textAlign: 'center', cursor: 'pointer', color: '#4490ff' }}>删除</span>
                                    </div>))
                            }
                        </div>
